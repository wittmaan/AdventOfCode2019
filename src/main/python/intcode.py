...
def process_opcode(x, input_instruction, init_pos=None):
    pos = 0 if not init_pos else init_pos
    program = x[:]
    output = []
    #print("input_instruction={}".format(input_instruction))
    ...
    while True:
        ...
        if opcode == Opcode.ADD:
            ...
            if not input_instruction:
                return program, output, pos

            program[program[pos + 1]] = input_instruction.pop()
            pos += 2
